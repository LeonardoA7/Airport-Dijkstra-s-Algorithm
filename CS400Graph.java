...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {     
      ...
      //Priority Queue that will find the next shortest path
      PriorityQueue<Path> running = new PriorityQueue<Path>();
      //Hash table that stores all visited vertex
      Hashtable<T, Vertex> visited = new Hashtable<T, Vertex>();
      //Hash table that stores all fastest paths
      Hashtable<T, Path> routes = new Hashtable<T, Path>(); // this is for the back end implementation
      
      Path starting = new Path(vertices.get(start));
      //Path's starting weight should be ZERO
      starting.distance = 0;
      running.add(starting);
      while(running.size() != 0) {
        //Get next shortest target Vertex
        Path current = running.peek();
        if(!visited.contains(current.end)) {
          //Mark the current Vertex and Path as visited
          visited.put(current.end.data, current.end);
          routes.put(current.end.data, current); // this is for the back end implementation
          //current end vertex has edges
          if(current.end.edgesLeaving != null) {
            for(int i = 0; i < current.end.edgesLeaving.size(); i++) {
              //if the target of the edge has not been visited, add it to the queue
              if(!visited.contains(current.end.edgesLeaving.get(i).target)) {
                Path temp = new Path(current, current.end.edgesLeaving.get(i));
                running.add(temp);
              }
            }
          }
